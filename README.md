...
1. **两个按钮在同一行**：输入框略缩短，给「主帐号获取云桌面」「子帐号获取云桌面」并排留位。
2. **同账号多卡片必须共用一份 state**（`--state acct_*` / 档案里同一会话文件），分文件会互踢 token。
3. **主/子不要混用错按钮**：用子帐号名却点主帐号登录会失败；反之亦然。选错后改点正确按钮重新获取列表即可。
4. **CLI 交互版**同样支持主/子登录；WebUI 与 CLI 对同一账号应避免同时登录。
5. 若云桌面被占用（桌面会话锁），可在卡片上使用 **「桌面登出」** 释放，再重新开始保活（CLI 对应 `logout` / `desktop-logout` 命令）。
